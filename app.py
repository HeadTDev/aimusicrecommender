...
# Custom CSS for Spotify-inspired look
st.markdown("""
    <style>
...
# User selects mood/genre or types a keyword
mood = st.selectbox(
...
# Song recommendation logic (simple filter - replace with ML as needed)
def recommend_song(mood, user_input, df):
    ...
    # Prioritize user input if given
    if user_input:
...
if st.button("Suggest a Song", key="suggest", help="Get a song recommendation!", 
    ...
    if isinstance(spotify_url, str) and spotify_url.startswith("https://open.spotify.com/track/"):
        # Show clickable link
        st.markdown(f"<br>[🎧 Listen on Spotify]({spotify_url})", unsafe_allow_html=True)
